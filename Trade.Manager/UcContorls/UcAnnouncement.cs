...
{
    public partial class UcAnnouncement : UcMaintenance
    {
        ...
        {
            var srv = new ReportSrv();
            var dt = srv.GetListAnnouncement("", "ID DESC");
        ...
        {
            var frm = new FrmAnnouncementModify { Model = null, Text = "新增公告明细" };
        ...
        private void BtnDelClick(object sender, EventArgs e)
        {
            if (dgvMain.SelectedRows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(condition))
                {
                    ...
                    var ws = new ReportSrv().DeleteListAnnouncement(condition);
                    if (ws)
                    {
                        DataBinding();
        ...
        private void BtnModifyClick(object sender, EventArgs e)
        {
            ...
            var frm = new FrmAnnouncementModify { Model = model, Text = "修改公告明细" };
            if (frm.ShowDialog() == DialogResult.Yes)
            {
                DataBinding();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private Announcement GetModel()
        {
            if (_id == 0) return null;

            var list = dgvMain.DataSource as Announcement[];
            if (list != null)
            {
