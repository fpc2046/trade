...
{
    public partial class UcBankAccountFound : UcMaintenance
    {
        public UcBankAccountFound()
        {
            InitializeComponent();
        ...
        {
            var srv = new ReportSrv();
            var dt = srv.GetListBankAccountFound("", "ID DESC");
        ...
        {
            var frm = new FrmBankAccountFoundModify { Model = null, Text = "新增银行帐号市场资金信息" };
        ...
        private void BtnDelClick(object sender, EventArgs e)
        {
            if (dgvMain.SelectedRows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(condition))
                {
                    ...
                    var ws = new ReportSrv().DeleteListBankAccountFound(condition);
                    if (ws)
                    {
                        DataBinding();
        ...
        private void BtnModifyClick(object sender, EventArgs e)
        {
            ...
            var frm = new FrmBankAccountFoundModify { Model = model, Text = "修改银行帐号市场资金信息" };
            if (frm.ShowDialog() == DialogResult.Yes)
            {
                DataBinding();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private BankAccountFound GetModel()
        {
            if (_id == 0) return null;

            var list = dgvMain.DataSource as BankAccountFound[];
            if (list != null)
            {
