...
{
    public partial class UcDeals : UcMaintenance
    {
        public UcDeals()
        {
            InitializeComponent();
        ...
        {
            var srv = new ReportSrv();
            var dt = srv.GetListDeal("", "ID DESC");
        ...
        {
            var frm = new FrmDealModify { Model = null, Text = "新增成交明细" };
        ...
        private void BtnDelClick(object sender, EventArgs e)
        {
            if (dgvMain.SelectedRows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(condition))
                {
                    ...
                    var ws = new ReportSrv().DeleteListDeal(condition);
                    if (ws)
                    {
                        DataBinding();
        ...
        private void BtnModifyClick(object sender, EventArgs e)
        {
            ...
            var frm = new FrmDealModify { Model = model, Text = "修改成交明细" };
            if (frm.ShowDialog() == DialogResult.Yes)
            {
                DataBinding();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private Deal GetModel()
        {
            if (_id == 0) return null;

            var list = dgvMain.DataSource as Deal[];
            if (list != null)
            {
