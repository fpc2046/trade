...
{
    public partial class UcFundFlows : UcMaintenance
    {
        public UcFundFlows()
        {
            InitializeComponent();
        ...
        {
            var srv = new ReportSrv();
            var dt = srv.GetListFundFlow("", "ID DESC");
        ...
        {
            var frm = new FrmFundFlowModify { Model = null, Text = "新增出入金流水" };
        ...
        private void BtnDelClick(object sender, EventArgs e)
        {
            if (dgvMain.SelectedRows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(condition))
                {
                    ...
                    var ws = new ReportSrv().DeleteListFundFlow(condition);
                    if (ws)
                    {
                        DataBinding();
        ...
        private void BtnModifyClick(object sender, EventArgs e)
        {
            ...
            var frm = new FrmFundFlowModify { Model = model, Text = "修改出入金流水" };
            if (frm.ShowDialog() == DialogResult.Yes)
            {
                DataBinding();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private FundFlow GetModel()
        {
            if (_id == 0) return null;

            var list = dgvMain.DataSource as FundFlow[];
            if (list != null)
            {
