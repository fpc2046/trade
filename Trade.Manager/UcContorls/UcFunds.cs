...
{
    public partial class UcFunds : UcMaintenance
    {
        public UcFunds()
        {
            InitializeComponent();
        ...
        {
            var srv = new ReportSrv();
            var dt = srv.GetListFund("", "ID DESC");
        ...
        {
            var frm = new FrmFundModify { Model = null, Text = "新增资金流水" };
        ...
        private void BtnDelClick(object sender, EventArgs e)
        {
            if (dgvMain.SelectedRows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(condition))
                {
                    ...
                    var ws = new ReportSrv().DeleteListFund(condition);
                    if (ws)
                    {
                        DataBinding();
        ...
        private void BtnModifyClick(object sender, EventArgs e)
        {
            ...
            var frm = new FrmFundModify { Model = model, Text = "修改资金流水" };
            if (frm.ShowDialog() == DialogResult.Yes)
            {
                DataBinding();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private Fund GetModel()
        {
            if (_id == 0) return null;

            var list = dgvMain.DataSource as Fund[];
            if (list != null)
            {
