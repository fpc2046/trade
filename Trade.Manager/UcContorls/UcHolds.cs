...
{
    public partial class UcHolds : UcMaintenance
    {
        public UcHolds()
        {
            InitializeComponent();
        ...
        {
            var srv = new ReportSrv();
            var dt = srv.GetListHold("", "ID DESC");
        ...
        {
            var frm = new FrmHoldModify { Model = null, Text = "新增持仓明细" };
        ...
        private void BtnDelClick(object sender, EventArgs e)
        {
            if (dgvMain.SelectedRows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(condition))
                {
                    ...
                    var ws = new ReportSrv().DeleteListHold(condition);
                    if (ws)
                    {
                        DataBinding();
        ...
        private void BtnModifyClick(object sender, EventArgs e)
        {
            ...
            var frm = new FrmHoldModify { Model = model, Text = "修改持仓明细" };
            if (frm.ShowDialog() == DialogResult.Yes)
            {
                DataBinding();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private Hold GetModel()
        {
            if (_id == 0) return null;

            var list = dgvMain.DataSource as Hold[];
            if (list != null)
            {
