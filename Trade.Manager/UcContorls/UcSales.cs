...
{
    public partial class UcSales : UcMaintenance
    {
        public UcSales()
        {
            InitializeComponent();
        ...
        {
            var srv = new ReportSrv();
            var dt = srv.GetListSale("", "ID DESC");
        ...
        {
            var frm = new FrmSaleModify { Model = null, Text = "新增成交明细" };
        ...
        private void BtnDelClick(object sender, EventArgs e)
        {
            if (dgvMain.SelectedRows.Count > 0)
            {
                ...
                if(!string.IsNullOrEmpty(condition))
                {
                    ...
                    var ws = new ReportSrv().DeleteListSale(condition);
                    if(ws)
                    {
                        DataBinding();
        ...
        private void BtnModifyClick(object sender, EventArgs e)
        {
            ...
            var frm = new FrmSaleModify { Model = model, Text = "修改平仓明细" };
            if (frm.ShowDialog() == DialogResult.Yes)
            {
                DataBinding();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private Sale GetModel()
        {
            if (_id == 0) return null;

            var list = dgvMain.DataSource as Sale[];
            if (list != null)
            {
