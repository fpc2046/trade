...
{
    public partial class UcTradeAccountFund : UcMaintenance
    {
        public UcTradeAccountFund()
        {
            InitializeComponent();
        ...
        {
            var srv = new ReportSrv();
            var dt = srv.GetListTradeAccountFund("", "ID DESC");
        ...
        {
            var frm = new FrmTradeAccountFundModify { Model = null, Text = "新增出入金操作" };
        ...
        private void BtnDelClick(object sender, EventArgs e)
        {
            if (dgvMain.SelectedRows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(condition))
                {
                    ...
                    var ws = new ReportSrv().DeleteListTradeAccountFund(condition);
                    if (ws)
                    {
                        DataBinding();
        ...
        private void BtnModifyClick(object sender, EventArgs e)
        {
            ...
            var frm = new FrmTradeAccountFundModify { Model = model, Text = "修改出入金操作" };
            if (frm.ShowDialog() == DialogResult.Yes)
            {
                DataBinding();
            }
        ...
        /// </summary>
        /// <returns></returns>
        private TradeAccountFund GetModel()
        {
            if (_id == 0) return null;

            var list = dgvMain.DataSource as TradeAccountFund[];
            if (list != null)
            {
